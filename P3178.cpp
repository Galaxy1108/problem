...
struct node {
    long long weight;        // 子树大小
    long long father;        // 父节点
    long long deep;          // 节点深度
    long long heavy_chil;    // 重儿子节点
    long long chain_top;     // 链顶节点
    long long dfso;          // 节点的DFS序（节点子树的dfs序左端点）
    long long dfso_end;      // 节点子树的dfs序右端点
} nodes[100005];       // 全局节点数组

long long dfso_node[100005];
...
bool updata_big(long long& comp_old, long long comp_new, long long& cond_old, long long cond_new) {
...
void pre_process(long long this_node) {
    long long this_heavy_chil = 0, max_weight = numeric_limits<long long>::min();
...
void build_chain(long long this_node, long long this_top, long long& dfs_order) {
...
void line_add(long long node_a,long long node_b,long long add_num){
...
long long line_query(long long node_a,long long node_b){
    long long anss = 0;
    while(nodes[node_a].chain_top != nodes[node_b].chain_top){
        if(nodes[nodes[node_a].chain_top].deep < nodes[nodes[node_b].chain_top].deep){
            swap(node_a, node_b);
...
void subtree_add(long long this_node,long long add_num){
    seg_add({nodes[this_node].dfso, nodes[this_node].dfso_end}, add_num, {1, n}, 1);
}

long long subtree_query(long long this_node){
    return seg_query({ nodes[this_node].dfso, nodes[this_node].dfso_end }, { 1, n }, 1);
}
...
int main() {
    ...
    pre_process(1);
    long long tree_weight = 1;
    build_chain(1, 1, tree_weight);
    for(long long i = 1; i <= n;i++){
    ...
    for (long long i = 1; i <= m;i++) {
        ...
        switch(opt){
            case 1:
                cin >> x >> z;
                seg_add({nodes[x].dfso, nodes[x].dfso}, z, {1, n}, 1);
                break;
            case 2:
                cin >> x >> z;
                subtree_add(x, z);
